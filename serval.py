...
class Tpl:
   ...
   def rotbroad(self, vsini=0, u=0):
      if vsini > 0:
         # broaden template
         self.wk, self.fk = rotbroad(self.wk0, self.fk0, vsini, u=u)

         # update funcarg for evaluation
         self.funcarg = self.initfunc(self.wk, self.fk)
   def rotbroad_int(self, vsini=0, u=0.6):
      if vsini > 0:
          # use new numerical integration instead of convolution
          res = rotbroad_int(self.wk0, self.fk0, vsini, b=self.bk0, u=u)
          if len(res) == 3:
              self.wk, self.fk, self.bk = res
          else:
              self.wk, self.fk = res
              self.bk = self.bk0
...
def rot_int_cmj(w, s, vsini, u=0.6, nr=10, ntheta=100, dif=0.0):
    """
    Rotational broadening using numerical integration on a log(λ) grid.

    Parameters
    ----------
    w     : log-wavelength grid (ln λ), uniformly spaced
    s     : spectrum or bad pixel map (can be float or bool)
    vsini : projected rotational velocity [km/s]
    u     : limb-darkening coefficient (default 0.6)
    nr    : radial sampling
    ntheta: azimuthal sampling resolution scaling
    dif   : differential rotation coefficient (default 0 = rigid rotation)

    Returns
    -------
    Broadened spectrum or flag map.
    """
    c_kms = 2.9979e5
    ns = np.zeros_like(s, dtype=float)  # Ensure float dtype even if s is bool
    tarea = 0.0
    dr = 1.0 / nr

    for j in range(nr):
        r = dr / 2.0 + j * dr
        nphi = max(1, int(ntheta * r))
        area = ((r + dr / 2.0)**2 - (r - dr / 2.0)**2) / nphi
        area *= (1.0 - u + u * np.cos(np.arcsin(r)))  # limb-darkening

        for k in range(nphi):
            phi = np.pi / nphi + k * 2.0 * np.pi / nphi

            if dif != 0.0:
                mu = np.cos(np.arcsin(r))
                vl = vsini * r * np.sin(phi) * (1.0 - dif/2 - dif/2 * np.cos(2 * np.arccos(r * np.cos(phi))))
            else:
                vl = vsini * r * np.sin(phi)

            # shift in log(λ): additive in v/c
            w_shifted = w + (vl / c_kms)
            ns += area * np.interp(w_shifted, w, s)
            tarea += area
...
def rotbroad_int(x, f, v, b=None, u=0.6, nr=10, ntheta=100, dif=0.0):
    """
    Broaden a spectrum by rotation using numerical integration.

    Parameters
    ----------
    x : np.ndarray
        Wavelength grid (assumed uniform in ln(lambda)).
    f : np.ndarray
        Spectrum flux (same length as x).
    v : float
        Projected rotational velocity vsini in km/s.
    b : np.ndarray or None
        Bad pixel map (same length as x).
    u : float
    ...
    -------
    x : np.ndarray
        Output wavelength grid (same as input).
    f : np.ndarray
        Broadened flux.
    b : np.ndarray (optional)
        Bad pixel map (broadened).
    """
    f_broadened = rot_int_cmj(x, f, v, u=u, nr=nr, ntheta=ntheta, dif=dif)

    if b is not None:
        b = rot_int_cmj(x, b, v, u=u, nr=nr, ntheta=ntheta, dif=dif)

    return (x, f_broadened) if b is None else (x, f_broadened, b)


def rotbroad(x, f, v, b=None, u=0):
    '''
    Broaden a spectrum by rotation including limb darkening.
    ...
    Returns
    -------
    x : Original ln(lambda) grid.
    f : Broadened spectrum (downsampled to original grid).
    '''
    x_orig = x.copy()  # save original grid for later downsampling

    # oversample (assumes oversampling is defined elsewhere)
    x, f = oversampling(x, f, 5)  # oversampling factor = 5

    dv = (x[1] - x[0]) * c  # velocity step [km/s]
    kmax = v / dv
    k = int(kmax)
    if k == 0:
        return x_orig, f[:len(x_orig)]  # or interpolate if needed

    # rotation broadening kernel with limb darkening
    xk = np.arange(-k, k + 1) / kmax
    A = 2*(1 - u)*np.sqrt(1 - xk**2) + 0.5*np.pi*u*(1 - xk**2)
    A /= np.sum(A)

    # apply convolution
    f_broad = np.convolve(f, A, mode='valid')
    x_broad = x[k:-k]  # matched size with 'valid' convolution

    # interpolate back to original grid (downsampling)
    interp_func = interp1d(x_broad, f_broad, bounds_error=False, fill_value='extrapolate')
    f_downsampled = interp_func(x_orig)

    return x_orig, f_downsampled
...
def oversampling(wavelength, flux, factor):
    import numpy as np
    from scipy.interpolate import interp1d

    """Dynamically oversample wavelength, flux, and error while keeping the original wavelengths"""
    
    delta_waves = wavelength[1:] - wavelength[:-1]
    num_points = factor  # Number of points per interval

    # Generate new wavelengths between existing points
    new_wavelengths = np.concatenate([
        wavelength[:-1] + (i / num_points) * delta_waves for i in range(1, num_points)
    ] + [wavelength])
    new_wavelengths = np.sort(new_wavelengths)

    # Interpolate flux and error onto new wavelength grid
    flux_interp = interp1d(wavelength, flux, kind='slinear', fill_value="extrapolate")
    # error_interp = interp1d(wavelength, error, kind='slinear', fill_value="extrapolate")

    new_flux = flux_interp(new_wavelengths)
    # new_error = error_interp(new_wavelengths)

    return new_wavelengths, new_flux #, new_error


def interpolate_ld_coeffs(teff_target):
    teff_grid = [2500, 3000, 3500, 4000, 4500]

    ld_data = {}
    for teff in teff_grid:
        filepath = f"/home/rvaras/data/vsini/ld_coefficients/teff{teff}_linear.dat"
        try:
            data = np.loadtxt(filepath)
            orders = data[:, 0].astype(int)
            u_coeffs = data[:, -1]  # 4th column (last)
            ld_data[teff] = (orders, u_coeffs)
        except OSError:
            print(f"File missing: {filepath}")
            continue

    all_teffs = sorted(ld_data.keys())
    if teff_target <= all_teffs[0]:
        return ld_data[all_teffs[0]][1]
    if teff_target >= all_teffs[-1]:
        return ld_data[all_teffs[-1]][1]

    # Find bounding Teffs
    for i in range(len(all_teffs) - 1):
        t1, t2 = all_teffs[i], all_teffs[i + 1]
        if t1 <= teff_target <= t2:
            u1 = ld_data[t1][1]
            u2 = ld_data[t2][1]
            weight = (teff_target - t1) / (t2 - t1)
            return (1 - weight) * u1 + weight * u2

    raise ValueError("Teff outside interpolation range")


def get_teff(obj, catalog_path="/home/rvaras/data/vsini/carmencita.107.csv"):
    df = pd.read_csv(catalog_path)

    # Find the row for the given object
    star_row = df[df["Karmn"] == obj]
    if star_row.empty:
        raise ValueError(f"Object {obj} not found in the catalog.")

    teff = star_row.iloc[0]["Teff_K"]

    # If Teff is not NaN, return it
    if pd.notna(teff):
        return teff

    # If Teff is NaN, find SpT
    spt = star_row.iloc[0]["SpT"]

    # Get first 20 entries with same SpT and non-null Teff
    same_spt_rows = df[(df["SpT"] == spt) & (df["Teff_K"].notna())].head(20)

    if same_spt_rows.empty:
        raise ValueError(f"No valid Teff found for SpT {spt}")

    median_teff = np.median(same_spt_rows["Teff_K"])

    return median_teff


def optivsini_int(va, vb, v_step, vtpl, x2, y2, e_y2, tpl, p=None, plot=3, o=0, obj='J00001+001'):
    """
    performs a mini CCF; the grid stepping
    returns best vsini and errors from parabola curvature
    """
    df = pd.read_csv("/home/rvaras/data/vsini/vsini_srv_v2.csv")
    value = df.loc[df["Karmn"] == obj, "vsini_this_work"]
    if not value.empty:
        vsini_value = value.values[0]
    else:
        vsini_value = np.nan
    
    # Set grid depending on vsini value
    if (not np.isnan(vsini_value)) and (vsini_value <= 4.0):
        vgrid = np.arange(0, 6.05, 0.05)  # grid 0 to 6 km/s with 0.05 step
    else:
        vgrid = np.arange(va, vb, v_step)
    
    nk = len(vgrid)

    u_list = interpolate_ld_coeffs(get_teff(obj))
    u_list[o-10]
    ...
    for k in range(nk):
        ...
        # broaden template
        calcspec.tpl.rotbroad_int(vgrid[k], u=0)
...
def optivsini(va, vb, v_step, vtpl, x2, y2, e_y2, tpl, p=None, plot=3, o=0, obj='J00001+001'):
    """
    performs a mini CCF; the grid stepping
    returns best vsini and errors from parabola curvature
    """
    ...
    u_list = interpolate_ld_coeffs(get_teff(obj))
    u=u_list[o-10]
    # x2, y2, e_y2 = oversampling(x2, y2, e_y2, 10)
    ...
    for k in range(nk):
        ...
        # broaden template
        calcspec.tpl.rotbroad(vgrid[k], u=u)
   ...
   for iterate in range(1, niter+1):
      ...
      if RV is not None:
         ...
         for o in corders:
            ...
            if vsiniauto or vsiniauto_int:
               ...
               if vsiniauto:
                   par, fModkeep = optivsini(0, vs_hi, vs_step, 0, #tplrv,
                                           x, y, yerr,
                                           TPL0[o],
                                           [1,0,0,0], plot=False, o=o, obj=obj)
               elif vsiniauto_int:
                   par, fModkeep = optivsini_int(0, vs_hi, vs_step, 0, #tplrv,
                                           x, y, yerr,
                                           TPL0[o],
                                           [1,0,0,0], plot=False, o=o, obj=obj)
            ...
            # apply the fit
            if not vsiniauto and not vsiniauto_int:
         ...
         if vsiniauto or vsiniauto_int:
            ...
            if subset.size == 0:
                ...
                filtered_subset = subset[(subset != 0.0) & (subset != 75.0)]
                med_vsini, p25, p75 = np.percentile(subset, [50, 25, 75])
                ...
                e_med_vsini = np.std(filtered_subset)  # 1.2533 * sd_vsini / np.sqrt(ok.sum())   # error of median [p213, Kendall 1948, Advanced Theory Of Statistics Vol 1, https://archive.org/details/in.ernet.dli.2015.57860/page/n223/mode/2up]
...
if __name__ == "__main__":
   ...
   argopt('-vsiniauto_int', help='automatic vsini computation. Requires external serval template (-tpl)', action='store_true')
